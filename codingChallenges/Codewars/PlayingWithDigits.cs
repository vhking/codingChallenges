...
namespace codingChallenges.CodeWars
{
    ...
    {
        public long digPower(int number, int power)
        {   
                
            var stringNum = number.ToString();
          
            var numList = stringNum.Select(digit => int.Parse(digit.ToString()));            
            long powerSum = 0;                       
            int powerIncrease = power;
                       
            foreach (var item in numList)
            {
                powerSum += (long)Math.Pow(item,powerIncrease);
                powerIncrease++;
            }         
           
            var result = powerSum/number;             
            if (powerSum  == result* number)
            {
                return result;
            }
